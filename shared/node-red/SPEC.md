...
The `shared/node-red/` module manages the lifecycle, authentication, settings injection, and code splitting configurations necessary to embed Node-RED tightly into the existing React Starter Kit backend cleanly and resiliently.
...
├── auth.js           # RskAuthStrategy: Passport adapter mapped to our JWT service
├── settings.js       # Dynamic configuration generation, node extraction
└── flow-splitter.js  # Node-RED event plugin for splitting/rebuilding flows
```
...
- **Extraction Magic**: 
  - `writeCustomNodes(userDir)`: Resolves `require.context('./nodes')`, extracting `getNodeJS()` and `getNodeHTML()`, and writing physical files to `<userDir>/nodes/rsk/`.
...
Configures a custom Passport strategy (`RskAuthStrategy`) to replace Node-RED's default internal auth.
...
3. **Role Binding**: Triggers the `hook` system (`app.get('hook')('auth.permissions')`) fetching the user's explicit permissions. Maps `nodered:admin` to scope `*` and `nodered:read` to scope `read`. Falling out on exceptions pushes users to `/admin`.
