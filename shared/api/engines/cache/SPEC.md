...
The singleton is registered on the DI container as `app.get('cache')` during engine autoloading. In `__DEV__` mode, this will be a `NoOpCache` regardless of the options.
...
- **Module `init(app)`**: Access via `app.get('cache')`. Use `withNamespace()` to scope by module.
