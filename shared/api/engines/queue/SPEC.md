...
The singleton is registered on the DI container as `app.get('queue')` during engine autoloading.
...
- **Module `init(app)`**: Access via `app.get('queue')`. Create channels for domain-specific job processing.
