...
Unlike other engines (`schedule`, `hook`), the worker engine exports only the `createWorkerPool` factory. Each module creates its own pool instance scoped to its own `require.context`. The engine is registered on the DI container as `app.get('worker')` which exposes the `{ createWorkerPool, WorkerError }` exports.
...
- **Module `init(app)`**: Access via `app.get('worker')` to get the factory, then create module-specific pools.
