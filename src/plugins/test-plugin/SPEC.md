...
> Read this document to understand WHAT features to build inside `src/plugins/test-plugin`. 
> Read `.cursorrules` and `README.md` to understand HOW to build them securely according to our architecture.
...
## 1. Database & Hooks (`/api` or `shared/plugin` Logic)
*Define what data needs to be modified or hooked into.*
...
*Define any isolated endpoints this plugin needs to expose.*
- **Method & Path:** [e.g., `POST /api/test-plugin/execute`]
...
*Define exactly what UI you want generated.*
...
*Define the required languages and keys.*
