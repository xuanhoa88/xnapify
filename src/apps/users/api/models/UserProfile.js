 ...
 */
export default async function createUserProfileModel(
  ...
  await hook('models').invoke('UserProfile:define', {
    attributes,
    container,
  });

  const UserProfile = connection.define('UserProfile', attributes, {
  ...
  UserProfile.associate = async function (models) {
    ...
    const hook = container.resolve('hook');
    await hook('models').invoke('UserProfile:associate', {
      models,
      model: UserProfile,
      container,
    });
