...
- Modifying an existing module or engine that has a colocated `README.md`
...
### Tier 1: Colocated `README.md` (Existing Code)
...
```
src/apps/<module>/
├── README.md                              # Current implementation (Tier 1)
...
```bash
cat src/apps/<module>/README.md        # Module spec
cat shared/api/engines/<engine>/README.md  # Engine spec
cat src/extensions/<ext>/README.md     # Extension spec
```

Every module/engine has one. If it doesn't, create one from `.agent/templates/README.template.md`.
...
> "Which section in the README.md does this task fulfill?"
...
<Good>
```javascript
// README.md §2: "POST /api/auth/login validates credentials and sets HTTP-only JWT cookies"
...
```
Tests trace directly to README.md section language
</Good>
...
5. **Update the spec:**
   - **Colocated:** Edit the `README.md` in the module/engine directory
...
**Current spec says:** "[exact quote from README.md §N]"
**Problem:** [why it doesn't work]
**Proposed change:** "[new wording]"
**Impact:** [which code files are affected]
```

> 🔴 **Never silently diverge from the spec.** Today's "obvious improvement" is tomorrow's "why doesn't this match the README.md?"

---

## Keeping README.md in Sync

Colocated `README.md` files describe **current implementation**. This means they must be updated whenever the implementation changes.
...
> 🔴 **Rule:** If you change code, change the README.md. They must stay in sync.
...
- [ ] Re-read the README.md (or design spec) with fresh eyes
...
Sometimes code exists without a README.md (new modules, hotfixes, legacy code).

### When This Happens

1. **Create a README.md.** Copy `.agent/templates/README.template.md` to the module directory.
...
4. **Create traceability.** Map existing code to the new README.md sections.
...
- Code with no README.md section covering it
- README.md section with no code implementing it
- Tests that don't reference spec language
- "While I'm here" additions
- README.md that doesn't match current code
- Silently changed behavior without spec amendment
- "The spec was wrong" without documented amendment
- Module/engine with no README.md at all
...
**TRACE:** Read `src/apps/auth/README.md` → §2 API Routes. The spec mentions `POST /api/auth/refresh-token` but does NOT mention a password reset endpoint. This is a **new requirement**.
...
**Current README.md §2 says:** Lists login, register, logout, OAuth, refresh-token, profile endpoints.
...
**After approval, update `src/apps/auth/README.md` §2:**
...
```javascript
// README.md §2: "Generates a time-limited reset token and sends reset email"
...
**RECORD:** Mark complete. README.md updated. Tests pass.
...
- **Colocated specs:** Every module/engine has a `README.md` in its root directory
- **README.md template:** `.agent/templates/README.template.md`
