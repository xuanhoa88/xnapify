...
Unlike Modules (which contain core domain logic), Plugins (`src/extensions/`) are isolated and modify behavior dynamically via Extension Slots and Event Hooks. They are loaded and unloaded during runtime via the Plugin Manager.

## Procedure: Developing a Plugin

1. **Directory Structure:** Create `src/extensions/[plugin-name]/`. Subdirectories include `api/`, `views/`, and `translations/`.
...
4. **IPC Pipelines:**
   ...
   - Frontend: `dispatch(executeIpc('plugin-name:action', payload))` (or via `useExtensionHooks`)
