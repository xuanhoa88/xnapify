...
### 1.4 Syntax Restrictions (ESLint-Enforced)

These are **banned by ESLint** and will fail lint. Flag immediately:

| ❌ Banned | ✅ Use Instead |
|-----------|----------------|
| `??` (nullish coalescing) | `x != null ? x : fallback` or `x \|\| fallback` |
| `??=` (nullish assignment) | `if (x == null) x = value` |
| `?.` (optional chaining) | `x && x.prop` or guard clause |
...
Defer to the **security-auditor** skill for deep checks. At minimum verify:
...
The code-reviewer checks that other skills' output is correct:

| Skill | What to Verify |
|-------|----------------|
| **module-developer** | Lifecycle hooks match phase order, route format correct, DI used properly |
...
- During `/update-code` workflow execution (mandatory before completing)
