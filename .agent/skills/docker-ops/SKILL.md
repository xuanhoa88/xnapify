...
Webpack's `DefinePlugin` replaces all `process.env.XNAPIFY_*` references with **literal strings at build time**. This means:

- `XNAPIFY_HOST`, `XNAPIFY_DB_URL`, `XNAPIFY_PORT`, etc. are **hardcoded** in the bundle
...
RUN XNAPIFY_HOST=0.0.0.0 XNAPIFY_DB_URL=postgresql://... npm run build

# Wrong: runtime override has no effect on baked values
environment:
  XNAPIFY_HOST: 0.0.0.0  # ignored if already baked
```

## Conventions

- **Single persistent volume**: `xnapify_data` mounted at `/app/data` — all writable dirs are subdirectories
- Writable dirs set via env vars: `XNAPIFY_UPLOAD_DIR`, `XNAPIFY_NODERED_HOME`, `XNAPIFY_EXTENSION_DIR`, `XNAPIFY_CACHE_DIR`, `XNAPIFY_FTS_DIR`
- Volume names use `xnapify_` prefix: `xnapify_data`, `xnapify_pg`, `xnapify_mysql`
- Network: `xnapify-net` (bridge driver)
- Container names: `xnapify-app`, `xnapify-postgres`, `xnapify-mysql`
...
2. Always set `XNAPIFY_HOST=0.0.0.0` in the build command
