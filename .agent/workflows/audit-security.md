...
Run a systematic security audit using the `security-compliance` skill across target modules, extensions, or the full codebase.

> **Prerequisite:** Read the `security-compliance` skill (`view_file .agent/skills/security-compliance/SKILL.md`) before starting. It defines the 12 audit domains and severity levels.
...
/audit-security oauth-google-plugin
...
After completing all steps, compile findings using the security-compliance skill report format:
...
1. If any 🔴 CRITICAL findings → fix immediately using `/modify` workflow
2. If 🟡 WARNING findings → create task items for remediation
3. Run the `code-review` skill on any fixes before committing

---

## See Also

- `/modify` — Apply fixes for audit findings with test verification
- `/add-module` — Module creation with built-in security patterns
- `/add-extension` — Extension creation with isolation guarantees
- `/commit` — Commit audit fixes using Conventional Commits
