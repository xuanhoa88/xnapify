...
- **Server**: Initialized in `src/server.js` and accessible via `container.resolve('ws')`.
...
```javascript
// @apps/chat/index.js
export default async function chatModule(container) {
  const ws = container.resolve('ws'); // Note: key is 'ws', not 'wss'
