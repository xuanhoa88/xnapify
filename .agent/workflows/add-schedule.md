...
schedule.register('marketing:batch-emails', '0 6 * * *', async () => {
  ...
  await queue('marketing').publish('dispatch-emails', {
...
- **Graceful Timeouts**: When `SIGTERM`/`SIGINT` fires, the schedule manager awaits your async Promises up to _5000ms_ using `Promise.allSettled`. It also cancels the task signals dynamically natively tracking memory allocations safely across the event-loop.
