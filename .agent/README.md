# rapid-rsk AI Architecture Guide

The `rapid-rsk` application is officially engineered to support intelligent developer assistance via a unified "Single Source of Truth" AI directory. Various AI agents (`Cursor`, `Claude`, `Gemini`) are configured to pull custom rules and workflows exclusively from the `.agent/` directory.
...
- **When is it used?** It is injected directly as the "System Prompt" into any AI session interacting with the codebase. The AI already knows the layout of `rapid-rsk` before you even ask it a question.
